...
public abstract class LayoutWindow {
     ...
     * max count of 1.
     *
     * @param UID unique identifier definition
     * @param parent window
     * @param name window name
     */
    public LayoutWindow(Screen screen, String UID, Element parent, String name) {
        this(screen, UID, parent, name, new Vector2f(230, 20), Align.Horizontal, 1);
    }
     ...
     * @param name window name
     */
    public LayoutWindow(Screen screen, String UID, Element parent, String name, Align align, int elementAlignMaxCount) {
        this(screen, UID, parent, name, new Vector2f(230, 20), align, elementAlignMaxCount);
    }
     ...
     */
    public LayoutWindow(Screen screen, String UID, Element parent, String name, Vector2f layoutGridSize) {
        this(screen, UID, parent, name, layoutGridSize, Align.Horizontal, 1);
    }

    /**
     * Generate a new window with.
     *
     * @param UID unique identifier definition
     * @param parent window
     * @param layoutGridSize element size on X and Y
...
//        window.setIgnoreMouse(true);
    ...
    protected final void show(VAlign vAlign, HAlign hAlign, boolean isHexGrid) {
        ...
        if (vAlign == null && hAlign == null) {
            ...
            if (vAlign != null) {
                switch (vAlign) {
                    case bottom:
                        position.y = screen.getHeight() - size.y;
                        break;
                    case top:
                        position.y = 0;
                        break;
                    default:
                        throw new UnsupportedOperationException(vAlign + " Not supported on the current context.");
                }
            }
            if (hAlign != null) {
                switch (hAlign) {
                    case left:
                        ...
                        break;
                    case right:
                        position.x = screen.getWidth() - size.x;
                        break;
                    default:
                        throw new UnsupportedOperationException(hAlign + " Not supported on the current context.");
                }
            }
            show(position);
        }
    ...
    private Vector2f getWindowSize() {
        ...
        return new Vector2f(column * layoutGridSize.x + (spacement * 2) + (isHexGrid && elementAlignMaxCount % 2 == 0 ? layoutGridSize.x / 2 : 0),
                (row + 0.9f) * (isHexGrid ? layoutGridSize.y * 2 / 3 : layoutGridSize.y) + (isHexGrid ? row * 2f : 0) + offsetY);
    }
    ...
    public void setVisible() {
        if (window != null) {
            ...
            LoggerFactory.getLogger(LayoutWindow.class).error("{} Can't be set to visible, it does not exist.", UID);
        }
    }
